...
public class WrappersApiTestDrive {
	...
	try {
	    ...
	    WikiTreePersonProfile churchillPersonByWikiTreeID = request.getPerson( "Churchill-4" );
	    WikiTreeApiUtilities.prettyPrintJsonThing( "Winston S. Churchill's person instance", churchillPersonByWikiTreeID );

	    WikiTreePersonProfile churchillPersonById = request.getPerson( "5589" );
	    WikiTreeApiUtilities.prettyPrintJsonThing( "Churchill's person instance by Person.Id", churchillPersonById );

	    WikiTreePersonProfile churchillProfile2 = request.getPersonProfile( "Churchill-4" );
	    ...
	    WikiTreeBiography bio = request.getBio( "Churchill-4" );
	    ...
	    WikiTreeAncestors ancestors = request.getAncestors( "Churchill-4", 4 );
	    ...
	    WikiTreeAncestors onilBoutinAncestors = request.getAncestors( "Boutin-148", 5 );
	    ...
	    WikiTreeApiUtilities.prettyPrintJsonThing( "relatives of 5589", relatives.getBasePeopleByPersonId().get( 5589L ).getChildren() );
	    WikiTreeApiUtilities.prettyPrintJsonThing( "everything by WikiTree ID", relatives.getBasePeopleByWikiTreeID() );

	    System.out.println(
	    	"timing stats for actual calls to remote WikiTree API server:  " + WikiTreeApiWrappersSession.getTimingStats()
	    );
